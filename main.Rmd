...
From the plot we can see that, as the number of bins increase, the MISE difference converges. The difference starts off high for the smallest number of bins, $5$. Using $10$ bins already substantially minimises the difference and as the number of bins grows, it only decreases minimally.

We also observe that the $MISE$ difference starts higher for the larger sample size $n=1000$, between $8$ and $10$, than for the smaller sample size $n=100$, around $6$.

This plot does not tell the whole story however, as the difference only tells us whether the original $MISE$ and perturbed $MISE$ are approaching each other, but not the actual $MISE$ values. As the next plot will show, the sample size $n$ does have an effect on the original $MISE$ value.


```{r}
plot_n_eps = function(mise_data, colours, subtitle = NULL) {
...
The absolute differences shown in the 4 plots correspond to the lines in the previous plot. By also plotting the MISE values for the original and perturbed histogram, we can compare them easier than if we had just plotted the absolute difference. For example, for a sample size $n = 1000$, we can see that the original MISE is close to $0$ whereas the MISE of the perturbed stabilises around $2$. For this reason, the perturbed $MISE$ and $MISE$ difference are almost identical for the larger sampl size $n=1000$.

Here we can also see that for the larger sample size of $n=1000$, the original $MISE$ is constant around $0$ whereas the original $MISE$ for the smaller sample size $n=100$ actually increases to around $1$. This explains why the $MISE$ difference for the smaller sample size $n=100$ in the first plot is actually lower than the one for the large sample size $n=1000$. 

It is due to the fact that the perturbed $MISE$ values converge at around $10$ bins for all sample sizes and epsilon values but the original $MISE$ value increasing for the smaller sample size $n=100$. Hence, we have a smaller difference between the original and perturbed $MISE$ for these hyperparameters.
