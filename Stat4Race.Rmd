...
Then the process is repeated for each iteration $i$ for all those values that still haven't found a solution but this time with an increase of the samples extracted for each variable.
...
for(i in 1:n.simulations) {
    for(m in M) {
        ...
        # store results
        row = data.frame(size=m, time=fin, it=i)
        row$values <- list(r)
        df = rbind(df, row)
    }
    fin<- Sys.time() - beg
